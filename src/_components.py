...
def Laplacian(V, E, X, m):
    ...
    for k in E.keys():
        ...
        c = 2*len(hyperedge) - 3
        if m:
            # connect the supreme with the infimum
...
def adjacency(edges, weights, n):
    ...
    edges, weights = np.array(edges), np.array(organized)
    adj = sp.coo_matrix((weights, (edges[:, 0], edges[:, 1])), shape=(n, n), dtype=np.float32)
    adj = adj + sp.eye(n)

    A = symnormalize(sp.csr_matrix(adj, dtype=np.float32))
    A = spmat2tensor(A)
    return A
